...
def dels(seq, del_rate, random):
    rnd = np.random.default_rng(random+del_rate)
    to_del = rnd.choice(len(seq), del_rate*len(seq)//100, replace=False)
    subseq = ''.join([v for i, v in enumerate(seq) if i not in to_del])
    return subseq

def inserts(seq, ins_rate, random=12345, max_inserts=5):
    aa = 'ACDEFGHIKLMNPQRSTVWY'
    ...
    for i, v in enumerate(seq):
        new_seq.append(v)
        if i in to_ins:
            rem_aa = list(set(aa) - set(seq[i]))
#             num_inserts = rnd.integers(2, max_inserts)
            num_inserts = 1
            for n in range(num_inserts):
                rand_aa = rnd.choice(list(rem_aa))
                new_seq.append(rand_aa)
    return ''.join(new_seq)


def gen_reads_indels(seq, aa_known, max_prob, ins_rate, del_rate, random, max_inserts=5):
    seq0 = dels(seq, del_rate, random)
    seq1 = inserts(seq0, ins_rate, random, max_inserts)
    return generate_reads(seq1, aa_known, max_prob)

cwd = os.getcwd()

dirs_to_make = ['temp', 'data', 'results', 'results/figs', 'results/scan_results']


tmp, data, results, figs, scan_results = [os.path.join(cwd, i) for i in dirs_to_make]


for d in [tmp, data, results, figs, scan_results ]:
    try:
        os.makedirs(d)
...
#         tp = [[0.8, 0.1, 0.1, 0.54, 0.46, 0.77, 0.23]] * M
        tp = [[0.9999, 0.00005, 0.00005, 0.54, 0.46, 0.77, 0.23]] * M
    else:
        tp = transition_prob
...
#     hits = pipeline.search_hmm(query=hmm, sequences=sequences)
    ...
    results = pd.DataFrame(temp_list, columns=['Query', 'Accession', 'E-value', 'Score'])
    return results
